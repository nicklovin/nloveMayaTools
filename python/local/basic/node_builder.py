...
from local.constants import node_blueprints
...
def create_node(node_key, name=None):
    ...
    try:
        node_name = node_blueprints.NODE_DICTIONARY[node_blueprints.NODE_NAME_DICTIONARY[node_key]]()
    ...
    if not name:
        if selection:
            ...
            name = cmds.objectType(node_name)

    print(node_name)
    print('%s_%s' % (name, node_blueprints.NODE_NAME_DICTIONARY[node_key]))

    node = cmds.rename(node_name, '%s_%s' % (name, node_blueprints.NODE_NAME_DICTIONARY[node_key]))
    return node
...
class NodeWidget(QtWidgets.QFrame):
    ...
    def __init__(self):
        ...
        node_widget.layout().addLayout(SplitterLayout())
        ...
        for node_ in sorted(node_blueprints.NODE_NAME_DICTIONARY.keys()):
            if node_[0] == node_[0].lower():
                self.node_type_combo.addItem(node_)
    ...
    def get_plugin_node_prefix(cls, plugin_node_key):
        plug_prefix = None
        for data in node_blueprints.PLUGIN_LIBRARIES.values():
    ...
    def _get_node_settings(self):
        node_key = node_blueprints.NODE_NAME_DICTIONARY[self.node_type_combo.currentText()]
    ...
    def _update_node_name(self):
        ...
        node_text = node_blueprints.NODE_NAME_DICTIONARY[node_key]
