...
def lock_hide(tx, ty, tz, rx, ry, rz, sx, sy, sz, v, objects=[], hide=True,
    ...
    param_attrs = zip(DEFAULT_ATTRS, [tx, ty, tz, rx, ry, rz, sx, sy, sz, v])
...
def reset_transforms(nodes, force=False):
    ...
    for node in nodes:
        for attr, val in zip(DEFAULT_ATTRS[:-1], DEFAULT_VALUES):  # Don't care about vis
            try:
...
def create_attr(attribute_name, attribute_type, input_object=None,
    ...
    Args:
        ...
        min_value (float): Sets a minimum value.  Only works if passed with a max
            value.
        max_value (float): Sets a maximum value.  Only works if passed with a min
            value.
...
class AddAttributesWidget(QtWidgets.QFrame):
    ...
    def __init__(self):
        ...
        add_attribute_widget.layout().addLayout(SplitterLayout())
