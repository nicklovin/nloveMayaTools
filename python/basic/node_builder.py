...
from ..widgets.common.splitter import SplitterLayout
...
def create_node(node_key, name=None):
    ...
    try:
        node = node_blueprints.NODE_DICTIONARY[node_blueprints.NODE_NAME_DICTIONARY[node_key]]()
    ...
    node.rename('%s_%s' % (name, node.NODE_NAME_DICTIONARY[node_key]))
    return node.name()
